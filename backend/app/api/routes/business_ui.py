...
async function delItem(id){if(!confirm('Delete?'))return;try{await fetch(API+'/'+id,{method:'DELETE'});}catch(e){try{await fetch(API.replace('?limit=50','')+'/'+id,{method:'DELETE'});}catch(e2){}}loadList();loadDash();}
async function updateSt(id,status){try{await fetch(API+'/'+id+'/status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:status})});}catch(e){try{await fetch(API+'/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:status})});}catch(e2){}}loadList();loadDash();}
async function delItem(id){if(!confirm('Delete?'))return;try{await fetch(API+'/'+id,{method:'DELETE'});}catch(e){}loadList();loadDash();}
async function updateSt(id,st){try{await fetch(API+'/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:st})});}catch(e){}loadList();loadDash();}
loadDash();loadList();
</script></body></html>"""
    return page
...
MODULES = {
    ...
    "assets": ("Assets", "&#128736;", "/assets", "assets", [("name","Asset Name","text",True),("type","Type","select:equipment,vehicle,furniture,electronics,machinery,solar_panel,camera,other",False),("brand","Brand","text",False),("model","Model","text",False),("serial_number","Serial No","text",False),("purchase_price","Purchase Price","number",False),("warranty_expiry","Warranty Expiry","date",False),("condition","Condition","select:new,good,fair,needs_repair",False)]),
    "projects": ("Projects", "&#128203;", "/projects", "projects", [("name","Project Name","text",True),("client_name","Client","text",False),("client_phone","Phone","tel",False),("description","Description","textarea",False),("budget_estimated","Budget","number",False),("start_date","Start Date","date",False),("end_date","End Date","date",False)]),
    "documents": ("Documents", "&#128196;", "/documents", "documents", [("title","Title","text",True),("type","Type","select:file,contract,agreement,kyc,invoice,certificate,template,other",False),("folder","Folder","text",False),("file_url","File URL","text",False),("description","Description","textarea",False)]),
}
...
@router.get("/{slug_or_id}/ai_employee", response_class=HTMLResponse)
async def ai_employee_page(slug_or_id: str):
    ...
    page = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>AI Employee - ' + org_name + '</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet"><style>' + base_css() + '.chat{max-height:calc(100vh - 240px);overflow-y:auto;padding:8px 0}.msg{margin-bottom:10px;max-width:85%}.msg-user{margin-left:auto;background:#6366f1;color:#fff;padding:10px 14px;border-radius:14px 14px 4px 14px;font-size:.82rem}.msg-ai{background:#fff;border:1px solid #f1f5f9;padding:10px 14px;border-radius:14px 14px 14px 4px;font-size:.82rem}.msg-ai .action{margin-top:6px;padding:6px 10px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;font-size:.7rem;color:#166534}.input-bar{position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:1px solid #e2e8f0;padding:10px 16px;max-width:640px;margin:0 auto;display:flex;gap:8px}.input-bar input{flex:1;padding:10px 14px;border:1px solid #e2e8f0;border-radius:10px;font-size:.85rem;outline:none}.input-bar button{padding:10px 16px;background:#6366f1;color:#fff;border:none;border-radius:10px;font-weight:700;cursor:pointer}.suggestions{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px}.sug{padding:6px 12px;background:#fff;border:1px solid #e2e8f0;border-radius:8px;font-size:.7rem;cursor:pointer;color:#6366f1}</style></head><body><div class="hd"><div class="hd-in"><h1>&#129302; AI Employee</h1><a href="#" style="color:#64748b;text-decoration:none;font-size:1.4rem" onclick="history.back();return false">&times;</a></div></div><div class="mn"><div class="suggestions" id="suggestions"></div><div class="chat" id="chat"><div class="msg msg-ai">Hi! I\'m your AI business assistant. I can help you:<br>&#8226; Create contacts, invoices, bookings<br>&#8226; Answer business questions<br>&#8226; Generate content ideas<br><br>Just type what you need!</div></div></div><div class="input-bar"><input id="input" placeholder="Type a command or question..." onkeydown="if(event.key===\'Enter\')send()"><button onclick="send()">Send</button></div><script>var API="/api/org/' + org_id + '/ai";async function loadSuggestions(){try{var r=await fetch(API+"/suggestions");var d=await r.json();document.getElementById("suggestions").innerHTML=(d.suggestions||[]).map(function(s){return \'<div class="sug" onclick="ask(this.textContent)">\'+s+\'</div>\';}).join("");}catch(e){}}loadSuggestions();function ask(text){document.getElementById("input").value=text;send();}async function send(){var input=document.getElementById("input");var q=input.value.trim();if(!q)return;input.value="";var chat=document.getElementById("chat");chat.innerHTML+=\'<div class="msg msg-user">\'+q+\'</div>\';chat.innerHTML+=\'<div class="msg msg-ai" id="typing" style="color:#94a3b8">Thinking...</div>\';chat.scrollTop=chat.scrollHeight;try{var r=await fetch(API+"/ask",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:q})});var d=await r.json();document.getElementById("typing").remove();var html=\'<div class="msg msg-ai">\'+d.answer.replace(/\\n/g,"<br>");if(d.action_taken)html+=\'<div class="action">&#10003; Action completed in \'+d.module+\'</div>\';html+=\'</div>\';chat.innerHTML+=html;}catch(e){document.getElementById("typing").remove();chat.innerHTML+=\'<div class="msg msg-ai" style="color:#ef4444">Error. Try again.</div>\';}chat.scrollTop=chat.scrollHeight;}</script></body></html>'
