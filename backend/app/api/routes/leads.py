...
async def public_search(query: str = ""):
    """Search Google Maps for businesses and check if we already have their website."""
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for place in places:
                ...
                is_lead = False
                try:
                    existing = db.table("leads").select("id").eq("business_name", name).limit(1).execute()
                    if existing.data:
                        is_lead = True
                        lead_id = existing.data[0]["id"]
                        ws = db.table("websites").select("slug").eq("lead_id", lead_id).not_.is_("slug", "null").limit(1).execute()
...
async def search_specific_business(query: str = Query(..., description="Business name + pincode/area")):
    ...
    if _slug:
        try:
            from app.services.whatsapp_auto import send_site_created_message
            await send_site_created_message(stored.get("business_name", ""), stored.get("phone", ""), _slug)
        except Exception:
            pass
