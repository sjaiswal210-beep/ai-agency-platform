...
MOBILE_CSS = """
<style>
.lightbox{display:none;position:fixed;inset:0;background:rgba(0,0,0,.95);z-index:9999;align-items:center;justify-content:center;padding:20px}.lightbox.open{display:flex}.lightbox img{max-width:85%;max-height:75vh;border-radius:12px;object-fit:contain;transition:opacity .2s ease}.lightbox .close{position:absolute;top:16px;right:20px;color:#fff;font-size:2rem;cursor:pointer;background:none;border:none;z-index:10}.lb-nav{position:absolute;top:50%;transform:translateY(-50%);width:40px;height:40px;background:rgba(255,255,255,.15);backdrop-filter:blur(8px);border:none;border-radius:50%;color:#fff;font-size:1.2rem;cursor:pointer;display:flex;align-items:center;justify-content:center}.lb-prev{left:12px}.lb-next{right:12px}.lb-counter{position:absolute;bottom:16px;left:50%;transform:translateX(-50%);color:rgba(255,255,255,.6);font-size:.75rem}
...
AD_SLOTS = """<button onclick="window.open('https://wa.me/?text='+encodeURIComponent('Check out: '+document.title+' '+location.href),'_blank')" class="share-float" style="position:fixed;bottom:100px;right:16px;width:44px;height:44px;border-radius:50%;background:#2563EB;border:none;display:none;align-items:center;justify-content:center;box-shadow:0 4px 14px rgba(37,99,235,.35);z-index:98;cursor:pointer;animation:wab 3s ease-in-out infinite" title="Share this website"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg></button>
<style>@media(max-width:768px){.share-float{display:flex!important;opacity:.35!important;transition:opacity .3s!important}.share-float:hover,.share-float:active{opacity:1!important}}</style><script data-cfasync="false">(function(){var ss=document.querySelector('.stats-section');if(!ss)return;var img=ss.style.backgroundImage;window.addEventListener('scroll',function(){var rect=ss.getBoundingClientRect();var wh=window.innerHeight;if(rect.top<wh&&rect.bottom>0){var pct=(wh-rect.top)/(wh+rect.height);ss.style.backgroundPosition='center '+(30-pct*60)+'%'}},{{passive:true}});})()</script><script data-cfasync="false">function shareWebsite(){var url=location.href;var text="Check out this business website: "+document.title+" "+url;window.open("https://wa.me/?text="+encodeURIComponent(text),"_blank");var t=document.createElement("div");t.style.cssText="position:fixed;bottom:80px;right:20px;background:#22c55e;color:#fff;padding:8px 16px;border-radius:50px;font-size:.75rem;font-weight:600;z-index:9999";t.textContent="Link copied!";document.body.appendChild(t);setTimeout(function(){t.remove()},2000)}}</script><div id="dashLoginModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;display:none;align-items:center;justify-content:center;padding:20px">
...
link.target="_blank";
link.onclick=function(){fetch("/api/ads/track",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({campaign_id:a.id,event_type:"ad_click",website_id:wid})});};
var img=document.createElement("img");
img.src=a.creative_url;
img.style.cssText="max-width:100%;max-height:90px;border-radius:8px";
img.alt="sponsored";
...
def generate_html(content: dict, template: str, lead: dict = None, website_id_override: str = "") -> str:
    ...
    if real_photos and len(real_photos) >= 4:
        ...
        gallery_display = real_photos + [
            images["gallery"][idx % len(images["gallery"])]
    ...
    for i, svc in enumerate(services[:6]):
        ...
        svc_cards.append(
            ...
            f'<p>{svc.get("description","")}</p><button onclick="showSvcPopup(this)" data-name="{svc.get("name","")}" data-desc="{svc.get("description","")}" style="display:inline-block;margin-top:10px;padding:6px 14px;background:var(--p);color:#fff;border:none;border-radius:6px;font-size:.72rem;font-weight:600;cursor:pointer">Explore &rarr;</button></div></article>'
        )
            ...
            if _pr.data:
                ...
                for p in _pr.data:
                    ...
                    prod_cards += (
                        ...
                        f'<div class="prod-actions"><a href="/api/store/{website_id}/store-page" class="prod-buy">+ Add to Cart</a></div>'
                        f'</div>'
                    )
                products_html = (
                    '<section class="section products-section" style="padding:48px 16px">' + '<style>.prod-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;max-width:760px;margin:0 auto}.prod-card{background:#fff;border-radius:16px;overflow:hidden;border:1px solid var(--line);transition:transform .2s,box-shadow .2s}.prod-card:hover{transform:translateY(-3px);box-shadow:0 8px 24px rgba(0,0,0,.1)}.prod-img{width:100%;height:140px;object-fit:cover;background:var(--soft)}.prod-body{padding:12px 14px 14px}.prod-name{font-size:.95rem;font-weight:700;margin-bottom:4px;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.prod-desc{display:none}.prod-price{font-size:1.05rem;font-weight:800;color:#00b4d8;margin-bottom:10px}.prod-actions{display:block}.prod-buy{display:block;width:100%;text-align:center;padding:11px;background:color-mix(in srgb,var(--p) 14%,transparent);color:var(--p);border:1px solid color-mix(in srgb,var(--p) 30%,transparent);border-radius:10px;font-size:.8rem;font-weight:700;text-decoration:none}.prod-buy:active{transform:scale(.97)}.prod-enq{display:none}@media(max-width:768px){.prod-grid{gap:10px}.prod-img{height:100px!important}.prod-body{padding:9px 10px 10px}.prod-name{font-size:.82rem}.prod-price{font-size:.92rem;margin-bottom:7px}.prod-buy{padding:8px;font-size:.72rem;border-radius:8px}}</style>' + '<div style="max-width:1100px;margin:0 auto">'
    ...
    if whatsapp_num:
        ...
        bottom_nav = (
            ...
            f'<a href="/api/store/{website_id}/store-page"><span class="bn-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M7 18c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H5.21l-.94-2H1zm16 16c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/></svg></span>Store</a>'
            f'<a href="{dir_url}" target="_blank"><span class="bn-icon">&#128205;</span>Maps</a>'
            '</div></div>'
        ) if phone else ""
...
.hero-pill{{display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);backdrop-filter:blur(6px);padding:5px 12px;border-radius:50px;font-size:.68rem;font-weight:500;color:rgba(255,255,255,.85);margin-bottom:16px}}.hero-offer{{display:inline-block;color:#fff;font-weight:600;font-size:.92rem;padding:0;margin-bottom:18px;opacity:.9}}
...
/* STATS */
.stats-section{{background:url('{real_photos[1] if len(real_photos) > 1 else (real_photos[0] if real_photos else about_img)}') center/cover;padding:48px 24px;position:relative;overflow:hidden;filter:none;background-attachment:fixed;-webkit-background-attachment:scroll}}.stats-section::before{{content:'';position:absolute;inset:0;background:rgba(10,15,30,.5);backdrop-filter:blur(4px) grayscale(.6) contrast(1.2)}}.stats-section::after{{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(0,0,0,.1) 25%,transparent 25%,transparent 50%,rgba(0,0,0,.1) 50%,rgba(0,0,0,.1) 75%,transparent 75%);background-size:3px 3px;pointer-events:none;opacity:.4;mix-blend-mode:overlay}}
.stats-grid{{position:relative;z-index:2;display:grid;grid-template-columns:repeat(4,1fr);gap:16px;max-width:1000px;margin:0 auto;text-align:center}}.stat-item{{color:#fff;text-shadow:0 2px 8px rgba(0,0,0,.5)}}.stat-number{{font-size:clamp(1.4rem,2.5vw,2rem);font-weight:800;text-shadow:0 2px 12px rgba(0,0,0,.6)}}.stat-label{{font-size:.82rem;opacity:.9;margin-top:4px;font-weight:500}}
...
.bottom-nav{{display:none;position:fixed;bottom:0;left:0;right:0;z-index:1000;background:linear-gradient(135deg,var(--p),var(--ps));padding:8px 12px calc(8px + env(safe-area-inset-bottom)) 12px;border-top:none;box-shadow:0 -4px 20px rgba(0,0,0,.25);backdrop-filter:blur(10px);border-radius:16px 16px 0 0}}
.bottom-nav-grid{{display:flex;justify-content:space-around;align-items:center;max-width:400px;margin:0 auto}}
.bottom-nav a{{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;text-decoration:none;color:rgba(255,255,255,.9);font-size:.62rem;font-weight:600;padding:8px 12px;min-height:48px;min-width:48px;border-right:1px solid rgba(255,255,255,.2);transition:all .2s;-webkit-tap-highlight-color:transparent}}.bottom-nav a:last-child{{border-right:none}}.bottom-nav a:hover,.bottom-nav a:active{{background:rgba(255,255,255,.15);border-radius:10px;transform:scale(.92)}}.bottom-nav a{{transition:all .2s cubic-bezier(.4,0,.2,1)}}
.bottom-nav a:nth-child(1){{color:#fff}}.bn-icon{{font-size:1.2rem;line-height:1;display:flex;align-items:center;justify-content:center;width:24px;height:24px}}.bn-icon svg{{width:18px;height:18px}}
...
.nav-cta{{padding:8px 14px!important;font-size:.75rem!important}}.hero-btns{{flex-direction:column!important;gap:10px!important;align-items:flex-start!important}}.btn-main,.btn-wa,.btn-outline{{padding:12px 24px!important;font-size:.88rem!important;width:auto}}
    ...
    html = (
        ...
        '</div>'
        f'<div style="background:rgba(255,255,255,0.05);padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,0.1);overflow:hidden"><h3 style="font-size:1rem;font-weight:600;margin-bottom:12px;color:#fff">Find Us</h3><iframe src="{maps_url}" width="100%" height="250" style="border:0;border-radius:10px" allowfullscreen loading="lazy"></iframe></div>'
        '</div></div></section>'
        f'{wa_link}'
        ''
        ''
        ...
        f'<div class="claim-modal" id="claimModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(8px)"><div style="background:rgba(15,23,42,.92);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.1);border-radius:20px;padding:24px;max-width:440px;width:100%;max-height:90vh;overflow-y:auto;position:relative;box-shadow:0 20px 60px rgba(0,0,0,.5)"><button onclick="document.getElementById(\'claimModal\').style.display=\'none\'" style="position:absolute;top:12px;right:14px;background:none;border:none;color:#94a3b8;font-size:1.3rem;cursor:pointer">&times;</button><h2 style="font-size:1.1rem;font-weight:800;color:#fff;margin-bottom:4px;text-align:center">\U0001f680 Claim Your Business</h2><p style="font-size:.72rem;color:#94a3b8;text-align:center;margin-bottom:16px">Get full control + premium growth tools</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px"><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:14px;text-align:center"><div style="font-size:.6rem;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Growth</div><div style="font-size:1.4rem;font-weight:900;color:#00e5ff">\u20b929<small style="font-size:.6rem;font-weight:400;color:#64748b">/mo</small></div><ul style="list-style:none;padding:0;margin:10px 0 0;font-size:.58rem;color:#94a3b8;text-align:left;line-height:1.8"><li>\U0001f310 Website</li><li>\U0001f6d2 Products</li><li>\U0001f4c8 Analytics</li><li>\U0001f4f7 Social Posts</li><li>\U0001f4f8 QR Code</li><li>\U0001f4c5 Daily Content</li><li>\U0001f381 Offers</li><li>\U0001f4cd Google Setup</li><li>\U0001f3a8 Logo</li><li>\u270e Edit Site</li><li>\U0001f517 Social Links</li><li>\U0001f5bc Gallery</li><li>\U0001f386 Festivals</li></ul></div><div style="background:rgba(124,58,237,.08);border:1.5px solid rgba(124,58,237,.3);border-radius:12px;padding:14px;text-align:center;position:relative"><div style="position:absolute;top:-8px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;font-size:.45rem;padding:2px 8px;border-radius:50px;font-weight:700">POPULAR</div><div style="font-size:.6rem;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Premium</div><div style="font-size:1.4rem;font-weight:900;color:#00e5ff">\u20b939<small style="font-size:.6rem;font-weight:400;color:#64748b">/mo</small></div><ul style="list-style:none;padding:0;margin:10px 0 0;font-size:.58rem;color:#94a3b8;text-align:left;line-height:1.8"><li>\U0001f310 Website</li><li>\U0001f6d2 Products</li><li>\U0001f4c8 Analytics</li><li>\U0001f4f7 Social Posts</li><li>\U0001f4f8 QR Code</li><li>\U0001f4c5 Daily Content</li><li>\U0001f381 Offers</li><li>\U0001f4cd Google Setup</li><li>\U0001f3a8 Logo</li><li>\U0001f3ac Promo Videos</li><li>\u270e Edit Site</li><li>\U0001f517 Social Links</li><li>\U0001f5bc Gallery</li><li>\U0001f386 Festivals</li><li>\U0001f4aa Custom Tools</li><li>\U0001f30f Custom Domain</li><li>\U0001f512 Priority Support</li></ul></div></div><a href="https://city-maps.online/api/panel/{website_id}" style="display:block;text-align:center;padding:12px;background:linear-gradient(135deg,#00e5ff,#0ea5e9);color:#020817;border-radius:10px;font-weight:700;font-size:.82rem;text-decoration:none;box-shadow:0 6px 20px rgba(0,229,255,.25);margin-bottom:8px">Claim Now \u2192</a><button onclick="document.getElementById(\'claimModal\').style.display=\'none\'" style="display:block;width:100%;padding:8px;background:none;border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#64748b;font-size:.7rem;cursor:pointer">Close</button></div></div>'
        
        f'<footer class="footer"><div style="max-width:900px;margin:0 auto;display:flex;flex-direction:column;align-items:center;gap:16px"><div style="font-weight:700;font-size:1rem;color:#fff">{business_name}</div><div style="font-size:.8rem;color:#64748b">Made with &#10084;&#65039; by <a href="https://city-maps.online" target="_blank" style="color:#00e5ff;text-decoration:none">City-Maps.online</a></div></div></footer>'
        ...
        '<script>var _galImgs=[];var _galIdx=0;document.body.insertAdjacentHTML("beforeend","<div class=lightbox id=lb><button class=close onclick=closeLb()>&times;</button><img id=lbImg><div class=lb-counter id=lbCount></div></div>");document.getElementById("lb").addEventListener("click",function(e){if(e.target===this)closeLb()});function openLb(s){_galImgs=Array.from(document.querySelectorAll(".gallery-item img")).map(function(i){return i.src});_galIdx=_galImgs.indexOf(s);if(_galIdx<0)_galIdx=0;showLbImg();document.getElementById("lb").classList.add("open");clearInterval(window._lbTimer);window._lbTimer=setInterval(function(){_galIdx=(_galIdx+1)%_galImgs.length;showLbImg()},2500)}function showLbImg(){var img=document.getElementById("lbImg");img.style.opacity="0";setTimeout(function(){img.src=_galImgs[_galIdx];img.style.opacity="1"},150);document.getElementById("lbCount").textContent=(_galIdx+1)+"/"+_galImgs.length}function closeLb(){document.getElementById("lb").classList.remove("open");clearInterval(window._lbTimer)};AOS.init({duration:650,once:true,offset:60});const nav=document.getElementById("mainNav");window.addEventListener("scroll",()=>{nav.classList.toggle("solid",scrollY>60)});if("serviceWorker"in navigator){navigator.serviceWorker.register("data:text/javascript,self.addEventListener(\'fetch\',e=>e.respondWith(fetch(e.request)))").catch(()=>{});}let deferredPrompt;window.addEventListener("beforeinstallprompt",e=>{e.preventDefault();deferredPrompt=e;const b=document.createElement("div");b.innerHTML=\'<div style="position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#fff;color:#333;padding:12px 20px;border-radius:50px;box-shadow:0 4px 20px rgba(0,0,0,.15);font-size:.85rem;font-weight:600;z-index:9999;cursor:pointer;display:flex;align-items:center;gap:8px" onclick="this.remove()">\\u2b07\\ufe0f Install App<\\/div>\';document.body.appendChild(b.firstChild);setTimeout(()=>{const el=document.querySelector("[onclick]");if(el)el.addEventListener("click",()=>{deferredPrompt.prompt();})},100)});</script>'
        f'{bottom_nav}'
        f'{AD_SLOTS.replace("WEBSITE_ID_PLACEHOLDER", website_id_override or "")}'
