...
.nav-cta{{padding:8px 14px!important;font-size:.75rem!important}}.hero-btns{{flex-direction:column!important;gap:10px!important;align-items:flex-start!important}}.btn-main,.btn-wa,.btn-outline{{padding:12px 24px!important;font-size:.88rem!important;width:auto}}
    ...
    html = (
        ...
        '<script>var _galImgs=[];var _galIdx=0;document.body.insertAdjacentHTML("beforeend","<div class=lightbox id=lb><button class=close onclick=document.getElementById(String.fromCharCode(108,98)).classList.remove(String.fromCharCode(111,112,101,110))>&times;</button><button class=lb-nav\ lb-prev onclick=lbPrev()>&#10094;</button><img id=lbImg><button class=lb-nav\ lb-next onclick=lbNext()>&#10095;</button><div class=lb-counter id=lbCount></div></div>");document.getElementById("lb").addEventListener("click",function(e){if(e.target===this)this.classList.remove("open")});function openLb(s){_galImgs=Array.from(document.querySelectorAll(".gallery-item img")).map(function(i){return i.src});_galIdx=_galImgs.indexOf(s);if(_galIdx<0)_galIdx=0;showLbImg();document.getElementById("lb").classList.add("open")}function showLbImg(){document.getElementById("lbImg").src=_galImgs[_galIdx];document.getElementById("lbCount").textContent=(_galIdx+1)+"/"+_galImgs.length}function lbPrev(){_galIdx=(_galIdx-1+_galImgs.length)%_galImgs.length;showLbImg()}function lbNext(){_galIdx=(_galIdx+1)%_galImgs.length;showLbImg()};AOS.init({duration:650,once:true,offset:60});const nav=document.getElementById("mainNav");window.addEventListener("scroll",()=>{nav.classList.toggle("solid",scrollY>60)});if("serviceWorker"in navigator){navigator.serviceWorker.register("data:text/javascript,self.addEventListener(\'fetch\',e=>e.respondWith(fetch(e.request)))").catch(()=>{});}let deferredPrompt;window.addEventListener("beforeinstallprompt",e=>{e.preventDefault();deferredPrompt=e;const b=document.createElement("div");b.innerHTML=\'<div style="position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#fff;color:#333;padding:12px 20px;border-radius:50px;box-shadow:0 4px 20px rgba(0,0,0,.15);font-size:.85rem;font-weight:600;z-index:9999;cursor:pointer;display:flex;align-items:center;gap:8px" onclick="this.remove()">\\u2b07\\ufe0f Install App<\\/div>\';document.body.appendChild(b.firstChild);setTimeout(()=>{const el=document.querySelector("[onclick]");if(el)el.addEventListener("click",()=>{deferredPrompt.prompt();})},100)});</script>'
        f'{bottom_nav}'
