...
<script>function toggleChat(){var p=document.getElementById("chatPanel");if(!p)return;if(p.style.display==="flex"){p.style.display="none"}else{p.style.display="flex";p.style.flexDirection="column"}}document.addEventListener("DOMContentLoaded",function(){var b=document.createElement("div");b.id="chatBubble";b.setAttribute("onclick","toggleChat()");b.innerHTML="&#128172;";b.style.cssText="position:fixed;bottom:70px;right:14px;width:54px;height:54px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 4px 20px rgba(99,102,241,.5);z-index:99999;font-size:24px;animation:chatPulse 2s ease infinite";var s=document.createElement("style");s.textContent="@keyframes chatPulse{0%,100%{box-shadow:0 4px 16px rgba(99,102,241,.4)}50%{box-shadow:0 4px 24px rgba(99,102,241,.7),0 0 0 8px rgba(99,102,241,.1)}}";document.head.appendChild(s);document.body.appendChild(b)});</script>
