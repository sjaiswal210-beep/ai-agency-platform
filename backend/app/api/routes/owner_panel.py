...
.stats{{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:12px;position:relative;z-index:1}}.stat{{background:rgba(255,255,255,.03);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:18px 8px;text-align:center;box-shadow:0 6px 24px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.05)}}.stat .n{{font-size:1.45rem;font-weight:800;background:linear-gradient(135deg,#00e5ff,#6366f1);-webkit-background-clip:text;-webkit-text-fill-color:transparent}}.stat .l{{font-size:.62rem;color:#94a3b8;margin-top:4px;font-weight:500}}
.section-title{{font-size:.66rem;font-weight:800;color:#7c8aa5;text-transform:uppercase;letter-spacing:.1em;margin:18px 0 9px;padding-left:4px;position:relative;z-index:1}}
.tools{{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;position:relative;z-index:1}}.tool{{background:rgba(255,255,255,.03);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:16px 6px;text-align:center;text-decoration:none;color:#fff;transition:all .2s;box-shadow:0 2px 10px rgba(0,0,0,.15),inset 0 1px 0 rgba(255,255,255,.04)}}.tool:active{{transform:scale(.95);box-shadow:0 1px 4px rgba(0,0,0,.2)}}.tool .emoji{{font-size:1.55rem;margin-bottom:6px;display:block}}.tool .name{{font-size:.68rem;font-weight:600;line-height:1.25}}.tool .desc{{display:none}}
...
<a href="/api/owner-broadcast/{website_id}" target="_blank" class="tool"><div class="emoji">&#128222;</div><div class="name">Voice Broadcast</div><div class="desc">Call your customers</div></a>
...
<div style="padding:8px 12px;position:relative;z-index:1;margin-bottom:8px;display:flex;gap:8px"><a href="/api/owner-broadcast/{website_id}" target="_blank" style="flex:none;display:flex;align-items:center;justify-content:center;width:44px;height:44px;background:linear-gradient(135deg,#f59e0b,#d97706);border-radius:12px;text-decoration:none;font-size:1.2rem;box-shadow:0 4px 12px rgba(245,158,11,.3)">&#128222;</a><a href="https://ai-agency-platform-blush.vercel.app/dashboard/''' + f'{slug}' + '''" target="_blank" style="display:block;text-align:center;padding:12px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border-radius:12px;text-decoration:none;font-weight:700;font-size:.82rem;box-shadow:0 4px 12px rgba(99,102,241,.3)">&#128187; Open Business Portal</a></div>
<div style="padding:8px 12px;position:relative;z-index:1;margin-bottom:8px;display:flex;gap:8px"><a href="/api/owner-broadcast/{website_id}" target="_blank" style="flex:none;display:flex;align-items:center;justify-content:center;width:44px;height:44px;background:linear-gradient(135deg,#f59e0b,#d97706);border-radius:12px;text-decoration:none;font-size:1.2rem;box-shadow:0 4px 12px rgba(245,158,11,.3)">&#128222;</a><a href="https://ai-agency-platform-blush.vercel.app/dashboard/' + slug + '" target="_blank" style="display:block;text-align:center;padding:12px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border-radius:12px;text-decoration:none;font-weight:700;font-size:.82rem;box-shadow:0 4px 12px rgba(99,102,241,.3)">&#128187; Open Business Portal</a></div>
<!-- chat bubble completely disabled -->
<div id="chatPanel" style="display:none!important" style="display:none;position:fixed;bottom:125px;right:12px;width:300px;max-width:85vw;height:360px;background:rgba(15,23,42,.97);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,.1);border-radius:16px;z-index:9999;flex-direction:column;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.5)">
...
<script>document.addEventListener('click',function(e){var a=e.target.closest('a[target="_blank"]');if(a&&a.href&&a.getAttribute('href')!=='#'){e.preventDefault();window.open(a.href,'_blank');}});function openTool(url){var m=document.getElementById('toolModal');m.style.display='flex';m.style.visibility='visible';document.getElementById('toolFrame').src=url;}function closeTool(){document.getElementById('toolModal').style.display='none';document.getElementById('toolFrame').src='';}function closeModals(){document.querySelectorAll("#editorPanel,#socialPanel,#galleryPanel").forEach(function(p){p.style.display="none"})}
...
def competitors_page(website_id: str):
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>Competitors</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:sans-serif;background:#0f172a;color:#fff;padding:16px;max-width:500px;margin:0 auto}}input,select,textarea{{font-size:16px!important}}.card{{background:rgba(255,255,255,.03);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:14px;margin-bottom:10px}}.btn{{display:block;width:100%;padding:12px;background:#6366f1;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:.85rem;cursor:pointer;text-align:center}}</style>{app_style}</head><body>
...
def crm_page(website_id: str):
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>CRM - {business_name}</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:sans-serif;background:#0f172a;color:#fff;padding:16px;max-width:500px;margin:0 auto}}input,textarea{{font-size:16px!important;width:100%;padding:10px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;margin-bottom:8px;outline:none}}.btn{{width:100%;padding:12px;background:#6366f1;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:.85rem;cursor:pointer}}</style>{app_style}</head><body>
...
def invoices_page(website_id: str):
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>Invoices - {business_name}</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:sans-serif;background:#0f172a;color:#fff;padding:16px;max-width:500px;margin:0 auto}}input,textarea,select{{font-size:16px!important;width:100%;padding:10px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;margin-bottom:8px;outline:none}}.btn{{width:100%;padding:12px;background:#22c55e;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:.85rem;cursor:pointer}}</style>{app_style}</head><body>
...
def inventory_page(website_id: str):
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>Inventory - {business_name}</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:sans-serif;background:#0f172a;color:#fff;padding:16px;max-width:500px;margin:0 auto}}input{{font-size:16px!important;width:100%;padding:10px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;margin-bottom:8px;outline:none}}.btn{{width:100%;padding:12px;background:#6366f1;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:.85rem;cursor:pointer}}</style>{app_style}</head><body>
