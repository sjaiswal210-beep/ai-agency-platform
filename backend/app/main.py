...
async function pubSearch(){{var biz=document.getElementById("sBiz").value.trim();var area=document.getElementById("sArea").value.trim();var country=document.getElementById("sCountry").value;if(!biz)return;var r=document.getElementById("searchResult");r.innerHTML="<p style='text-align:center;color:#64748b;font-size:.75rem'>Searching Google Maps...</p>";try{{var query=biz+(area?" "+area:"")+(country?" "+country:"");var resp=await fetch("/api/leads/public-search?query="+encodeURIComponent(query));var data=await resp.json();if(data.results&&data.results.length>0){{var html="<div class='glass' style='padding:12px;margin-top:8px;max-height:280px;overflow-y:auto'><p style='font-size:.7rem;color:#64748b;margin-bottom:8px'>"+data.results.length+" businesses found. Select yours:</p>";data.results.forEach(function(b){{html+="<div onclick='selectBiz(this)' data-name='"+b.name.replace(/'/g,"")+"' data-phone='"+(b.phone||"")+"' data-address='"+(b.address||"").replace(/'/g,"")+"' data-category='"+(b.category||biz)+"' style='display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border:1px solid rgba(255,255,255,.06);border-radius:8px;margin-bottom:6px;cursor:pointer;transition:all .2s' onmouseover='this.style.borderColor=\"rgba(0,229,255,.3)\"' onmouseout='this.style.borderColor=\"rgba(255,255,255,.06)\"'><div><p style='font-size:.78rem;font-weight:600;color:#e2e8f0'>"+b.name+"</p><p style='font-size:.65rem;color:#64748b;margin-top:2px'>"+(b.address||"").substring(0,50)+(b.phone?" &middot; "+b.phone:"")+"</p></div><span style='font-size:.65rem;color:#00e5ff;font-weight:600'>Select</span></div>";}});html+="</div>";r.innerHTML=html;}}else{{r.innerHTML="<div class='glass' style='padding:14px;margin-top:8px;text-align:center'><p style='font-size:.78rem;color:#e2e8f0;margin-bottom:6px'>No results found</p><p style='font-size:.7rem;color:#64748b'>Try a different name or area</p></div>";}}}}catch(e){{r.innerHTML="<div class='glass' style='padding:12px;margin-top:8px;text-align:center'><p style='font-size:.75rem;color:#ef4444'>Search failed. Try again.</p></div>";}}}}
async function selectBiz(el){{var name=el.dataset.name;var phone=el.dataset.phone;var address=el.dataset.address;var category=el.dataset.category;var r=document.getElementById("searchResult");r.innerHTML="<div class='glass' style='padding:14px;margin-top:8px;text-align:center'><p style='font-size:.78rem;color:#00e5ff;font-weight:600'>Creating website for "+name+"...</p><p style='font-size:.7rem;color:#64748b;margin-top:4px'>This takes about 30 seconds</p></div>";try{{var resp=await fetch("/api/leads/public-create-site",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{business_name:name,phone:phone,address:address,category:category}})}});var data=await resp.json();if(data.slug){{r.innerHTML="<div class='glass' style='padding:16px;margin-top:8px;text-align:center'><p style='font-size:.85rem;color:#22c55e;font-weight:700;margin-bottom:6px'>Website Created!</p><a href='https://"+data.slug+".city-maps.online' target='_blank' style='display:inline-block;padding:10px 20px;background:linear-gradient(135deg,#00e5ff,#00b4d8);color:#020817;border-radius:8px;font-weight:700;font-size:.75rem;margin-top:6px'>Visit "+data.slug+".city-maps.online &rarr;</a>"+(data.whatsapp_sent?"<p style='font-size:.65rem;color:#22c55e;margin-top:8px'>&#10003; WhatsApp notification sent to owner</p>":"")+"</div>";}}else{{r.innerHTML="<div class='glass' style='padding:12px;margin-top:8px;text-align:center'><p style='font-size:.75rem;color:#f59e0b'>"+( data.message||"Website creation in progress. Check back soon!")+"</p></div>";}}}}catch(e){{r.innerHTML="<div class='glass' style='padding:12px;margin-top:8px;text-align:center'><p style='font-size:.75rem;color:#ef4444'>Failed to create. Please try again.</p></div>";}}}}
// Parallax on mouse move
document.addEventListener('mousemove',function(e){{var x=(e.clientX/window.innerWidth-.5)*20;var y=(e.clientY/window.innerHeight-.5)*20;var g=document.querySelectorAll('.bg-depth .glow-1,.bg-depth .glow-2,.bg-depth .glow-3');g.forEach(function(el,i){{var f=(i+1)*0.5;el.style.transform='translate('+x*f+'px,'+y*f+'px)'}});}});
</script>
</body></html>'''
    return HTMLResponse(content=html)
