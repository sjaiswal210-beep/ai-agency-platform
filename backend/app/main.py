...
<style>
*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:Inter,sans-serif;background:#050816;color:#fff;overflow-x:hidden}}a{{text-decoration:none;color:inherit}}
...
/* HERO */
.hero{{min-height:90vh;display:flex;align-items:center;position:relative;padding:80px 20px 60px;overflow:hidden}}
...
/* PRICING */
.p-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}}.p-card{{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:20px;text-align:center;transition:all .2s}}.p-card:hover{{border-color:rgba(0,229,255,.2)}}.p-card.pop{{border-color:rgba(124,58,237,.4);box-shadow:0 0 20px rgba(124,58,237,.08)}}.p-card h3{{font-size:.75rem;color:#64748b;margin-bottom:4px}}.p-card .price{{font-size:1.8rem;font-weight:900;margin-bottom:2px}}.p-card .price small{{font-size:.7rem;color:#64748b}}.p-card ul{{list-style:none;text-align:left;margin:12px 0}}.p-card li{{font-size:.72rem;color:#94a3b8;padding:3px 0;padding-left:14px;position:relative}}.p-card li::before{{content:'\2022';position:absolute;left:0;color:#00e5ff;font-size:1rem;line-height:1}}.p-btn{{display:block;padding:9px;border-radius:8px;font-weight:700;font-size:.78rem;margin-top:12px}}.p-btn.fill{{background:linear-gradient(135deg,#7c3aed,#00e5ff);color:#fff}}.p-btn.ghost{{border:1px solid rgba(255,255,255,.1);color:#e2e8f0}}
...
<section class="sec" id="demos"><div class="sec-label">Live Examples</div><div class="sec-title">Businesses Already Online</div><div class="sec-desc">Real businesses using City Maps right now</div>
<div class="demo-grid">{examples_html}</div></section>
