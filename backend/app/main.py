...
.orb{{position:absolute;border-radius:50%;filter:blur(80px);animation:float 18s ease-in-out infinite}}
.orb-1{{width:50%;height:50%;top:-15%;left:-10%;background:radial-gradient(ellipse,rgba(0,229,255,.12),transparent 60%)}}
.orb-2{{width:45%;height:45%;bottom:-15%;right:-10%;background:radial-gradient(ellipse,rgba(124,58,237,.15),transparent 60%);animation-delay:3s}}
.orb-3{{width:35%;height:35%;top:40%;left:30%;background:radial-gradient(ellipse,rgba(236,72,153,.08),transparent 55%);animation-delay:6s}}
@keyframes float{{0%,100%{{transform:translate(0,0) scale(1)}}50%{{transform:translate(20px,-20px) scale(1.05)}}}}

/* Glass */
.glass{{background:rgba(255,255,255,.03);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.06);border-radius:16px}}

/* Nav */
.nav{{position:fixed;top:0;left:0;right:0;z-index:50;padding:16px 24px;display:flex;align-items:center;justify-content:space-between;max-width:1100px;margin:0 auto}}
...
.nav-btn{{padding:8px 16px;font-size:.72rem;font-weight:600;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;transition:all .2s}}.nav-btn:hover{{background:rgba(255,255,255,.1)}}

/* Hero */
.hero{{min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center;padding:100px 24px 60px;position:relative;z-index:1}}
.hero-grid{{position:absolute;inset:0;background-image:linear-gradient(rgba(0,229,255,.02) 1px,transparent 1px),linear-gradient(90deg,rgba(0,229,255,.02) 1px,transparent 1px);background-size:60px 60px;mask-image:radial-gradient(ellipse 80% 50% at 50% 0%,#000 70%,transparent 100%);-webkit-mask-image:radial-gradient(ellipse 80% 50% at 50% 0%,#000 70%,transparent 100%)}}
.hero-glow{{position:absolute;top:0;left:50%;transform:translateX(-50%);width:700px;height:400px;background:radial-gradient(ellipse,rgba(0,229,255,.08),rgba(124,58,237,.05),transparent 65%);filter:blur(60px)}}
.hero-inner{{position:relative;z-index:2;max-width:700px}}
.badge{{display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);backdrop-filter:blur(6px);padding:8px 16px;border-radius:50px;font-size:.72rem;font-weight:600;color:#00e5ff;margin-bottom:24px}}
.badge .dot{{width:7px;height:7px;border-radius:50%;background:#22c55e;box-shadow:0 0 8px #22c55e;animation:pulse 2s infinite}}
...
.btn-primary{{padding:14px 28px;background:linear-gradient(135deg,#00e5ff,#00b4d8);color:#050816;font-weight:700;font-size:.85rem;border-radius:12px;box-shadow:0 8px 30px rgba(0,229,255,.25);transition:all .25s}}.btn-primary:hover{{transform:translateY(-2px);box-shadow:0 12px 40px rgba(0,229,255,.35)}}
.btn-ghost{{padding:14px 28px;border:1px solid rgba(255,255,255,.12);color:#e2e8f0;font-weight:600;font-size:.85rem;border-radius:12px;transition:all .2s}}.btn-ghost:hover{{background:rgba(255,255,255,.05)}}
.hero-search{{max-width:420px;margin:24px auto 0;display:flex;gap:5px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:4px}}.hero-search input{{flex:1;background:transparent;border:none;padding:10px 14px;color:#fff;font-size:.82rem;outline:none}}.hero-search input::placeholder{{color:#4b5563}}.hero-search button{{background:linear-gradient(135deg,#00e5ff,#00b4d8);color:#050816;border:none;padding:10px 18px;border-radius:7px;font-weight:700;font-size:.75rem;cursor:pointer;transition:all .2s}}.hero-search button:hover{{transform:scale(1.02)}}#searchResult{{max-width:420px;margin:8px auto 0}}
.hero-badge-bottom{{margin-top:32px;display:inline-flex;align-items:center;gap:8px;font-size:.75rem;font-weight:600;color:#94a3b8}}
.hero-badge-bottom span{{color:#00e5ff}}

/* Stats */
.stats{{padding:28px 24px;border-top:1px solid rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.04);position:relative;z-index:1}}
...
/* Sections */
.sec{{padding:80px 24px;max-width:1000px;margin:0 auto;position:relative;z-index:1}}
...
.feat-card{{padding:24px;border-radius:16px;transition:all .3s}}.feat-card:hover{{border-color:rgba(0,229,255,.2);transform:translateY(-3px)}}
.feat-icon{{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;margin-bottom:14px;background:rgba(0,229,255,.08);border:1px solid rgba(0,229,255,.15)}}
...
.cat-item{{text-align:center;padding:16px 8px;border-radius:12px;transition:all .2s}}.cat-item:hover{{border-color:rgba(0,229,255,.2);transform:translateY(-2px)}}
.cat-item .emoji{{font-size:1.6rem;margin-bottom:6px}}
.cat-item .name{{font-size:.7rem;font-weight:600}}.cat-item .cnt{{font-size:.6rem;color:#4b5563;margin-top:2px}}

/* Journey */
.journey-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}}
.step-card{{text-align:center;padding:20px;border-radius:16px}}
.step-card .icon{{font-size:1.8rem;margin-bottom:10px}}
...
.stars{{color:#f59e0b;font-size:.85rem;margin-bottom:10px;letter-spacing:2px}}
...
.price-card{{padding:28px;border-radius:18px;display:flex;flex-direction:column;position:relative}}
.price-card.pop{{border-color:rgba(0,229,255,.3);box-shadow:0 0 30px rgba(0,229,255,.06)}}
...
/* CTA */
.cta{{text-align:center;padding:80px 24px;position:relative;z-index:1}}
.cta-glow{{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:400px;height:200px;background:radial-gradient(ellipse,rgba(0,229,255,.06),transparent 60%);pointer-events:none}}
...
<div class="hero-badge-bottom"><span>&#128640;</span> Boost Your Business <span style="color:#475569">|</span> <span>&#128222;</span> 24/7 Support</div>
</div>
</section>
...
<div class="feat-grid">
<div class="feat-card glass"><div class="feat-icon">&#127760;</div><h3>Professional Website</h3><p>Beautiful responsive site with services, gallery, map, reviews, and contact.</p></div>
<div class="feat-card glass"><div class="feat-icon">&#128172;</div><h3>WhatsApp Commerce</h3><p>One-tap order button. Customers enquire and buy directly on WhatsApp.</p></div>
<div class="feat-card glass"><div class="feat-icon">&#128205;</div><h3>Google Maps Visibility</h3><p>Show up when people search near you. Full Google profile optimization.</p></div>
<div class="feat-card glass"><div class="feat-icon">&#128722;</div><h3>Product Catalog</h3><p>Showcase products with photos, prices, and Buy Now button.</p></div>
<div class="feat-card glass"><div class="feat-icon">&#128200;</div><h3>Real-Time Analytics</h3><p>Track visitors, calls, WhatsApp clicks, and orders in real-time.</p></div>
...
<div class="cat-grid">
<div class="cat-item glass"><div class="emoji">&#127858;</div><div class="name">Restaurants</div><div class="cnt">28+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#128135;</div><div class="name">Salons</div><div class="cnt">22+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#127947;</div><div class="name">Gyms</div><div class="cnt">15+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#129658;</div><div class="name">Clinics</div><div class="cnt">18+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#127976;</div><div class="name">Hotels</div><div class="cnt">12+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#128717;</div><div class="name">Retail</div><div class="cnt">20+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#128247;</div><div class="name">Photographers</div><div class="cnt">10+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#127968;</div><div class="name">Real Estate</div><div class="cnt">8+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#127891;</div><div class="name">Schools</div><div class="cnt">6+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#9728;</div><div class="name">Solar</div><div class="cnt">5+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#9878;</div><div class="name">Lawyers</div><div class="cnt">4+ sites</div></div>
<div class="cat-item glass"><div class="emoji">&#9749;</div><div class="name">Cafes</div><div class="cnt">14+ sites</div></div>
</div></section>
...
<div class="step-card glass"><div class="icon">&#128221;</div><div class="step-num">Step 01</div><h3>Tell Us Your Business</h3><p>Share your business name and location. That is all we need.</p></div>
<div class="step-card glass"><div class="icon">&#9889;</div><div class="step-num">Step 02</div><h3>We Build Your Site</h3><p>We create a stunning website with SEO, products, and all features in 5 minutes.</p></div>
<div class="step-card glass"><div class="icon">&#128640;</div><div class="step-num">Step 03</div><h3>Go Live on Google</h3><p>Your site goes live instantly. Customers find you on Google Maps.</p></div>
<div class="step-card glass"><div class="icon">&#128176;</div><div class="step-num">Step 04</div><h3>Receive Orders</h3><p>Customers call, WhatsApp, and order directly. Watch your business grow.</p></div>
</div></section>
...
<!-- CTA -->
<section class="cta"><div class="cta-glow"></div>
...
<script>
async function searchBiz(){{var q=document.getElementById("bizSearch").value.trim();if(!q)return;var r=document.getElementById("searchResult");r.innerHTML="<p style='text-align:center;color:#64748b;font-size:.75rem;margin-top:8px'>Searching...</p>";try{{var slug=q.toLowerCase().replace(/[^a-z0-9\s-]/g,"").replace(/[\s]+/g,"-");var resp=await fetch("/api/preview/by-slug/"+slug);if(resp.ok){{r.innerHTML="<div style='background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.2);border-radius:10px;padding:12px;text-align:center;margin-top:8px'><p style='color:#22c55e;font-size:.8rem;font-weight:600'>Found!</p><a href='https://"+slug+".city-maps.online' target='_blank' style='color:#00e5ff;font-size:.75rem'>Visit "+slug+".city-maps.online &rarr;</a></div>";}}else{{throw new Error("nf");}}}}catch(e){{r.innerHTML="<div style='background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:16px;margin-top:8px;text-align:center'><p style='font-size:.82rem;margin-bottom:8px;color:#e2e8f0'>Business not found</p><p style='font-size:.72rem;color:#64748b;margin-bottom:12px'>Want a free business website? Fill details below:</p><form onsubmit='submitReq(event)' style='display:flex;flex-direction:column;gap:6px;max-width:300px;margin:0 auto'><input id=reqName value=\""+q+"\" placeholder='Business Name' required style='padding:9px 12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;font-size:.75rem;outline:none'><input id=reqDesc placeholder='Business Type (Salon, Gym, etc)' required style='padding:9px 12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;font-size:.75rem;outline:none'><input id=reqPhone placeholder='WhatsApp Number' required style='padding:9px 12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;font-size:.75rem;outline:none'><input id=reqAddr placeholder='Address / City' required style='padding:9px 12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#fff;font-size:.75rem;outline:none'><button type=submit style='background:linear-gradient(135deg,#00e5ff,#00b4d8);color:#050816;padding:10px;border:none;border-radius:8px;font-weight:700;font-size:.75rem;cursor:pointer;margin-top:4px'>Request My Website</button></form></div>";}}}}
async function submitReq(e){{e.preventDefault();try{{await fetch("/api/website-requests",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{business_name:document.getElementById("reqName").value,description:document.getElementById("reqDesc").value,phone:document.getElementById("reqPhone").value,city:document.getElementById("reqAddr").value}})}});document.getElementById("searchResult").innerHTML="<div style='background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.2);border-radius:10px;padding:12px;text-align:center;margin-top:8px'><p style='color:#22c55e;font-size:.8rem;font-weight:600'>Submitted! We will create your website and notify you soon.</p></div>";}}catch(err){{alert("Failed");}}}}
</script>
</body></html>'''
    return HTMLResponse(content=html)
