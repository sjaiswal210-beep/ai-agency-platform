async function pubSearch(){
  ...
  try{
    ...
    if(data.results&&data.results.length>0){
      var html="<div class='glass' style='padding:12px;margin-top:8px;max-height:280px;overflow-y:auto'><p style='font-size:.7rem;color:#64748b;margin-bottom:8px'>"+data.results.length+" businesses found. Select yours:</p>";
      data.results.forEach(function(b){
        html+="<div onclick=\"selectBiz(this)\" data-name=\""+b.name.replace(/'/g,"")+"\" data-phone=\""+(b.phone||"")+"\" data-address=\""+(b.address||"").replace(/'/g,"")+"\" data-category=\""+(b.category||biz)+"\" style='display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border:1px solid rgba(255,255,255,.06);border-radius:8px;margin-bottom:6px;cursor:pointer;transition:all .2s' onmouseover=\"this.style.borderColor='rgba(0,229,255,.3)'\" onmouseout=\"this.style.borderColor='rgba(255,255,255,.06)'\"><div><p style='font-size:.78rem;font-weight:600;color:#e2e8f0'>"+b.name+"</p><p style='font-size:.65rem;color:#64748b;margin-top:2px'>"+(b.address||"").substring(0,50)+(b.phone?" &middot; "+b.phone:"")+"</p></div><span style='font-size:.65rem;color:#00e5ff;font-weight:600'>Select</span></div>";
      });
      html+="</div>";
      r.innerHTML=html;
    }else{
...
async function selectBiz(el){
  ...
  var r=document.getElementById("searchResult");
  r.innerHTML="<div class='glass' style='padding:14px;margin-top:8px;text-align:center'><p style='font-size:.78rem;color:#00e5ff;font-weight:600'>Creating website for "+name+"...</p><p style='font-size:.7rem;color:#64748b;margin-top:4px'>This takes about 30 seconds</p></div>";
  try{
