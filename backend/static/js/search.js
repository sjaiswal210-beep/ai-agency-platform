    ...
    if(data.results&&data.results.length>0){
      ...
      data.results.forEach(function(b){
        if(b.has_website && b.slug){
          ...
          html+="<div style='display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border:1px solid rgba(34,197,94,.2);background:rgba(34,197,94,.04);border-radius:8px;margin-bottom:6px'><div><p style='font-size:.78rem;font-weight:600;color:#e2e8f0'>"+b.name+"</p><p style='font-size:.65rem;color:#64748b;margin-top:2px'>"+(b.address||"").substring(0,50)+"</p></div><a href='https://"+b.slug+".city-maps.online' target='_blank' style='font-size:.65rem;color:#22c55e;font-weight:700;background:rgba(34,197,94,.1);padding:6px 12px;border-radius:6px;text-decoration:none'>View Site &rarr;</a></div>";
        } else {
          // No website yet - show select button
          html+="<div onclick=\"selectBiz(this)\" data-name=\""+b.name.replace(/"/g,'&quot;')+"\" data-phone=\""+(b.phone||"")+"\" data-address=\""+(b.address||"").replace(/"/g,'&quot;')+"\" data-category=\""+(b.category||biz)+"\" style='display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border:1px solid rgba(255,255,255,.06);border-radius:8px;margin-bottom:6px;cursor:pointer;transition:all .2s' onmouseover=\"this.style.borderColor='rgba(0,229,255,.3)'\" onmouseout=\"this.style.borderColor='rgba(255,255,255,.06)'\"><div><p style='font-size:.78rem;font-weight:600;color:#e2e8f0'>"+b.name+"</p><p style='font-size:.65rem;color:#64748b;margin-top:2px'>"+(b.address||"").substring(0,50)+(b.phone?" &middot; "+b.phone:"")+"</p></div><span style='font-size:.65rem;color:#00e5ff;font-weight:600;background:rgba(0,229,255,.08);padding:6px 12px;border-radius:6px'>Create Site</span></div>";
        }
      });
      html+="</div>";
      r.innerHTML=html;
...
async function selectBiz(el){
  ...
  var r=document.getElementById("searchResult");
  r.innerHTML="<div class='glass' style='padding:14px;margin-top:8px;text-align:center'><p style='font-size:.78rem;color:#00e5ff;font-weight:600'>&#9889; Creating website for "+name+"...</p><p style='font-size:.7rem;color:#64748b;margin-top:4px'>This takes about 30 seconds. Please wait.</p><div style='margin-top:12px;width:40px;height:40px;border:3px solid rgba(0,229,255,.2);border-top:3px solid #00e5ff;border-radius:50%;animation:spin 1s linear infinite;margin:12px auto'></div></div><style>@keyframes spin{to{transform:rotate(360deg)}}</style>";
  try{
